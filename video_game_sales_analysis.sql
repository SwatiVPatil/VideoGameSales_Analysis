...
SELECT 
    CONCAT(
        'North America Sales: ', ROUND(SUM(NorthAmerica), 2), 'M, ',
        'Europe Sales: ', ROUND(SUM(Europe), 2), 'M, ',
        'Japan Sales: ', ROUND(SUM(Japan), 2), 'M, ',
        'Rest of World Sales: ', ROUND(SUM(Rest_of_World), 2), 'M, ',
        'Global Sales: ', ROUND(SUM(Global_sale), 2), 'M'
    ) AS Total_Sales
FROM 
    video_game_sales;

# North America Sales: 2400.51M, Europe Sales: 1347.63M, Japan Sales: 605.46M,
#  Rest of World Sales: 393.74M, Global Sales: 4746.98M
